...
gui.add(options, 'scale', .1, 10).onChange(draw)
gui.add(options, 'lemniscate').onChange(draw)
...
function draw() {
  ...
  p = guilloche(0, options.scale, options.R, options.r, options.p, options.lemn_t, options.f, options.lemniscate)
  ctx.moveTo(p.x, p.y)
  for (var j = 1; j < options.steps; j++) {
    theta = (2 * Math.PI) * (j / options.steps)
    p = guilloche(theta, options.scale, options.R, options.r, options.p, options.lemn_t, options.f, options.lemniscate)
    ctx.lineTo(p.x, p.y)
  }
