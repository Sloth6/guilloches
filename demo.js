...
function draw() {
  ...
  p = guilloche(0, options.scale, options.R, options.r, options.p, options.t, options.f)
  ctx.moveTo(p.x, p.y)
  for (var j = 1; j < options.steps; j++) {
    theta = (2 * Math.PI) * (j / options.steps)
    p = guilloche(theta, options.scale, options.R, options.r, options.p, options.t, options.f)
    ctx.lineTo(p.x, p.y)
  }
